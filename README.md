...
Целевая схема recipes-домена включает уже реализованные `recipes`, `cuisines`, `tags`, `recipe_tags`, `meal_types` и `recipe_meal_types`; `reviews` пока отсутствуют в реализации.
...
Успешный ответ использует полный формат поста, включая автора и комментарии.
...
- каждое переданное поле должно быть непустой строкой;
- неизвестные поля отклоняются с `400`;
- некорректный `id` отклоняется с `400`.
...
Изменить текст комментария. Тело: `{ text }`.

- изменить комментарий может только его автор;
- если комментарий не найден → 404;
- если пользователь не является автором → 403;
- успешный ответ использует тот же формат комментария, что и список комментариев.
